...
public class ColorChangingBehavior implements ColorLightBehavior {
    ...
    public static int animate(final ItemStack stack) {
        ...
        final float p = de.lucalabs.fairylights.main.util.MathHelper.mod(Util.getMillis() * (20.0F / 1000.0F) * (colors.size() / 960.0F), colors.size());
        final int i = (int) p;
    ...
    private float get(final float[] values, final float delta) {
        final float p = this.powered ? de.lucalabs.fairylights.main.util.MathHelper.mod(Util.getMillis() * (20.0F / 1000.0F) * this.rate, values.length) : 0.0F;
        final int i = (int) p;
